...
def main(argv):
    input_twt_file_name="data/syd_postcode_list.json"
    input_abs_file_name="data/pre_2016Census_G19_NSW_POA.csv"
    ...
    with open(input_abs_file_name, 'rb') as csvfile:
        ...
        for row in list_syd_data:
            for postcode in twt_data:
                if str(postcode) in row[0] and "POA_CODE" not in row[0] :
                    row[0]=row[0].replace("POA","")
                    row[3]="%.4f" % float(row[3])
                    syd_data+=[{"postcode":row[0],"volunteer_percentage":row[3]}]
